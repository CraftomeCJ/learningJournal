...
<!-- Learn to combine Low Leverage Activities into High Leverage activities with unavoidable low leverage activities in I life. -->
...
<!-- [x] I will say LOVE I to my wife for 3 times during the whole day in my office today -->
...
<!--learn To identify my three highest-leverage activities in my work, 
    ...
    Third => ask myself ==> the same question ==> If I could do only one more thing on I list of key tasks, which would be the second activity that contributes the most value to my company?
    last => ask myself again ==> ask Iself what the third most important task is.-->
...
## Unplanned Work
<!-- This is where I write down work that comes up. For example, need me to run a quick errand or my co-worker needs help. My goal should be to finish and record these activities as fast as possible so I can go back to my Outcome-Based Goals ==> the work I planned to get done. -->
    ...
    - by creating a test plans that lists the program's input and its expected result
      - **How a test plan looks like with example:**
        ...
        - **Expected Result:**
          ...
          - total amount: $11.50
        - **Actual Result:**
        - One test is not enough, take my time to develop 4 sets of sample data to test the program, and try to think of as many scenarios as possible for how people
...
function TipCalculator(): TipCalculator {
  let billAmount: number = 0;
  let tipRate: number =0;
  let tip: number = 0;
  let total: number = 0;
  ...
  function main(): void {
    ...
    billAmount = convert(readline.question());
    prompt("What is the tip rate?");
    tipRate = convert(readline.question());
    compute();
    display();
  }
  main();
...
ask During this time ask Iself:
